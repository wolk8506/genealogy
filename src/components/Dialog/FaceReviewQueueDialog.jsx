...
async function persistPhotoFaces(currentItem, nextFaces, allPhotos) {
  ...
  const updatedEntry = {
    ...
    faceScanNoFaces:
      normalized.length === 0 ? existing?.faceScanNoFaces : false,
  };
...
export default function FaceReviewQueueDialog({
  allPeople: allPeopleProp = [],
}) {
  const theme = useTheme();
  const isDark = theme.palette.mode === "dark";

  const open = useFaceReviewStore((s) => s.open);
  const closeReview = useFaceReviewStore((s) => s.closeReview);
  ...
  const allFaceIds = useMemo(
    () => (currentItem?.faces || []).map((f) => f.id),
    [currentItem?.faces],
  );
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      ...
      const nextQueue = buildReviewQueue(photos);
      setQueue(nextQueue);
      setPhotoIndex(0);
      setFaceIndex(0);
      setDrawMode(false);
      setPendingCount(
        nextQueue.reduce((sum, item) => sum + item.pendingFaceIds.length, 0),
      );
    } finally {
      setLoading(false);
    }
  ...
  const totalPending = useMemo(
    () => queue.reduce((sum, item) => sum + item.pendingFaceIds.length, 0),
    [queue],
  );
  ...
  const syncPendingCount = (nextQueue) => {
    setPendingCount(
      nextQueue.reduce((sum, item) => sum + item.pendingFaceIds.length, 0),
    );
  };
  ...
  const updateQueueFaces = (nextFaces, imageSize) => {
    ...
    nextQueue[photoIndex] = {
      ...nextQueue[photoIndex],
      faces: normalized,
      pendingFaceIds: getPendingFaceIds(normalized),
      imageSize: imageSize || nextQueue[photoIndex].imageSize,
    };
    setQueue(nextQueue);
    syncPendingCount(nextQueue);
    return nextQueue;
  };
  ...
  const afterSave = async (nextFaces, { goNextPhoto = false } = {}) => {
    cancelLayoutPersist();
    const allPhotos = await window.photoAPI.getAllGlobal();
    await persistPhotoFaces(currentItem, nextFaces, allPhotos);
    updateQueueFaces(nextFaces);

    const normalized = normalizeFaces(nextFaces);
    const pendingOnPhoto = getPendingFaceIds(normalized);

    if (goNextPhoto || (autoNextPhoto && pendingOnPhoto.length === 0)) {
      if (photoIndex < queue.length - 1) {
        setPhotoIndex(photoIndex + 1);
        setFaceIndex(0);
      }
    ...
    if (faceIndex >= normalized.length) {
      setFaceIndex(Math.max(0, normalized.length - 1));
    }
  };
  ...
  const handlePreviewLoad = (e) => {
    ...
    if (
      currentItem.imageSize?.width === naturalWidth &&
      currentItem.imageSize?.height === naturalHeight
    ) {
      return;
    }
    const nextQueue = [...queue];
  ...
  const handleDetectFaces = async () => {
    ...
    try {
      ...
      if (detected.length === 0) {
        addNotification({
          title: "Лица",
          message: "На фото не найдено новых лиц",
          type: "info",
          category: "faceReview",
        });
        return;
      ...
      setFaceIndex(Math.max(0, nextFaces.length - suggestedDetected.length));
      addNotification({
        title: "Лица",
        message: `Добавлено лиц: ${detected.length}`,
        type: "success",
        category: "faceReview",
      });
    } catch (e) {
  ...
  const handleDeleteFace = async (faceId) => {
    setSaving(true);
    try {
      const nextFaces = currentItem.faces.filter((f) => f.id !== faceId);
      await afterSave(nextFaces);
    } finally {
      setSaving(false);
    }
          ...
          <Stack direction="row" sx={{ height: 630 }}>
            ...
            >
              ...
              >
                {totalPending === 0 && (
                  <Alert
                    severity="success"
                    sx={{ py: 0.2, fontSize: "0.8rem", borderRadius: "10px" }}
                  >
                    Все лица подписаны.
                  </Alert>
                )}
                ...
                >
                  ...
                  {currentItem?.faces?.length > 0 ? (
                            ...
                            >
                              <Fade in={showSuggestion} timeout={300}>
                                ...
                                >
                                  ...
                                  {/* Кнопка Подтвердить (выровнена с кнопкой удаления) */}
                    ...
                    >
                      Нет рамок. Нажмите «Новая рамка» или «Найти лица».
                    </Typography>
                  )}
                </Box>
                ...
                >
                  ...
                  <Tooltip title="Авто-переход к след. фото после подписи всех лиц">
                    <Button
                      size="small"
                      variant={autoNextPhoto ? "contained" : "outlined"}
                      color={autoNextPhoto ? "primary" : "inherit"}
                      onClick={() => setAutoNextPhoto((v) => !v)}
                      sx={{
                        ...macButtonStyle,
                        ml: "auto",
                      }}
            ...
            <Stack flex={1} spacing={1.5} sx={{ p: 2 }}>
              ...
              >
                ...
                {totalPending > 0 && ` · без подписи: ${totalPending}`}
                {" · "}
